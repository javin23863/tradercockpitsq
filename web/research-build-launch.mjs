...
function renderLaunchGate(gate, configuration, execution, job, detail = "") {
  ...
  gate.innerHTML = `
    ...
    <p>The server will wrap the exact approved Build-Task1.xml in SQX's Task-rooted loadconfig .cfx, stage it inside the verified runtime, freshly reverify launcher/config identity, then submit only <code>loadconfig → start</code>.</p>
