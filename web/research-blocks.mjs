...
async function bindBlocksInspector() {
  ...
  try {
    ...
    const nodeCount = blocks.producer_configuration ? countNativeNodes(blocks.producer_configuration) : 0;
    workspace.innerHTML = `<details class="native-tree"><summary><span class="native-tree-title">${escapeHtml("Native rule / block space")}</span><span class="native-tree-meta">${nodeCount} native nodes · exact SQX <code>Blocks</code> subtree</span></summary>${renderNativeBuilderBlocks(blocks)}</details>`;
  } catch (error) {
    if (myGeneration !== generation || !specificationRoute() || !workspace.isConnected) return;
