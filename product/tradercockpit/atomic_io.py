...
file. Each write goes to a temp file in the same directory, is flushed and fsync'd,
then atomically renamed over the target with ``os.replace``. A per-path in-process
lock serializes writers within one process.
"""
