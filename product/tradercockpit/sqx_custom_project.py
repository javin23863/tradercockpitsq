...
not execute tasks; the canonical TraderCockpit workflow service consumes it.
"""
...
SQX_CUSTOM_PROJECT_OBSERVED_TASK_KINDS = frozenset(
    {"Build", "Retest", "ClearDatabanks", "GoToTask", "Optimize", "AutomaticPortfolioBuilder"}
)
...
def _read_archive_snapshot(path: Path) -> bytes:
    if not path.is_file():
        raise SqxCustomProjectTopologyError(
            "custom_project_missing", f"SQX Custom Project is missing: {path}"
        )
    try:
        return path.read_bytes()
    except OSError as exc:
        raise SqxCustomProjectTopologyError(
            "custom_project_unreadable", f"SQX Custom Project could not be read: {path}"
        ) from exc


def _task_from_xml(*, kind: str, index: int, entry_name: str, root: ElementTree.Element) -> SqxCustomProjectTask:
    clear_databanks: tuple[str, ...] = ()
    goto_target_label: str | None = None
        ...
        with ZipFile(BytesIO(archive_snapshot)) as archive:
            ...
            for entry_name in entries:
                ...
                by_index[index] = _task_from_xml(
                    kind=kind, index=index, entry_name=entry_name, root=root
                )
...
def read_sqx_custom_project_topology(sqx_home: Path | str | None, project: str) -> SqxCustomProjectTopology:
...
def custom_project_topology_record(sqx_home: Path | str | None, project: str) -> dict[str, object]:
