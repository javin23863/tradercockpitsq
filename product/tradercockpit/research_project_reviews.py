"""Retain exact saved Custom Project graphs and all project-bank inventories.
...
from .sqx_custom_project import SQX_BUILD, _read_topology, _resolved_project_archive, _databank_name, _parse_xml, _task_config_bindings, _project_databanks
...
def _bank_inputs(store, home, project, bank, inventory):
    _physical(home, bank)
    if bank.exists() and not bank.is_dir():
        raise ResearchCustodyError("project_review_input_invalid", "A configured databank is not a directory.")
    inputs = []
    for path in sorted(bank.glob("*.sqx"), key=lambda item: item.name):
        ...
        member = inventory.rows.get((project, bank.name, path.name))
...
def _snapshot(store, sqx_home, trusted_launcher_sha256, project, databank, inventory):
    ...
    root = home / "user" / "projects" / project / "databanks"
    _physical(home, root)
    task_banks = {}
    with ZipFile(BytesIO(graph)) as archive:
        ...
        config = _parse_xml(archive.read("config.xml"), "config.xml")
        order = list(_task_config_bindings(config))
        declared = {_databank_name(node.get("name")) for node in config.findall("./Databanks/Databank")}
        for task in tasks:
            xml = _parse_xml(archive.read(task.entry_name), task.entry_name)
            # Preserve native role names; a target bank is not necessarily a task result.
            task_banks[task.entry_name] = [
                {"role": node.get("name", ""), "databank": _databank_name(node.get("value"))}
                for node in xml.findall("./Databanks/Databank") if node.get("value")]
            task_banks[task.entry_name].extend({"role": "ClearDatabanks", "databank": _databank_name(name)} for name in task.clear_databanks)
    ...
    # Path equality follows the host filesystem's case rules; retain native spelling
    # on tasks without inventorying the same Windows bank twice.
    paths = dict.fromkeys(_project_databanks(home, project, snapshot=graph))
    for rows in task_banks.values():
        for row in rows:
            paths.setdefault(root / row["databank"], None)
    selected = root / _databank_name(databank)
    if selected not in paths:
        raise ResearchCustodyError("project_review_bank_missing", "Select a databank belonging to this saved project.")
    banks = [{"name": name, "declared": name in declared,
              "storage": "present" if (root / name).is_dir() else "not_created",
              "inputs": _bank_inputs(store, home, project, root / name, inventory)}
             for name in sorted(path.name for path in paths)]
    inputs = next(bank["inputs"] for bank in banks if root / bank["name"] == selected)
    snapshot = {"project": project, "databank": databank,
        "scope": "saved_graph_and_project_banks", "source_build": SQX_BUILD,
        ...
        "tasks": [{"index": task.native_task_index, "entry": task.entry_name,
                   ...
                   "active": task.active, "banks": task_banks[task.entry_name]} for task in tasks],
        "inputs": inputs, "banks": banks,
    ...
    if any(row["binding"] != "exact" for bank in banks for row in bank["inputs"]):
        snapshot["gaps"].append("Some saved inputs have no matching admitted Candidate revision.")
    return snapshot, graph


def _review_inputs(snapshot):
    # Older immutable reviews retain their original selected-bank scope.
    return [row for bank in snapshot["banks"] for row in bank["inputs"]] if "banks" in snapshot else snapshot["inputs"]
...
def _record(store, entity):
    ...
    expected.update(EvidenceRef.parse(row["archive_ref"]) for row in _review_inputs(payload["snapshot"]) if row["archive_ref"])
    ...
    try:
        ...
        with candidate_admission_batch(store) as inventory:
            ...
            if store._read_current(entity) is None:
                ...
                evidence = {graph_ref} | {EvidenceRef.parse(row["archive_ref"]) for row in _review_inputs(snapshot) if row["archive_ref"]}
