"""Native SQX System Parameter Permutation execution and immutable custody.

TraderCockpit does not implement the permutation algorithm. The current
installed Retester ``project.cfx`` remains the executable specification. This
adapter requires the producer-owned ``OptProfileSysParamPermutation`` profile
to already exist, preserves its native settings, enables only that profile in
an isolated snapshot when necessary, then runs the existing trusted Retester
task-1 gateway.

Process completion proves execution/custody only. Until an authoritative SQX
outcome seam is observed, no robustness pass/fail verdict is inferred.
"""
...
ROBUSTNESS_METHOD_SYSTEM_PARAMETER_PERMUTATION = "OptProfileSysParamPermutation"
...
def _exact_child(parent: ElementTree.Element, name: str, code: str) -> ElementTree.Element:
    ...
    if len(matches) != 1:
        raise ResearchRobustnessError(code, f"native System Parameter Permutation profile requires exactly one {name} element")
    return matches[0]
...
def _task_profile(
    ...
    *,
...
) -> tuple[ElementTree.Element, ElementTree.Element, dict[str, str]]:
    try:
        ...
        raise ResearchRobustnessError(
            "robustness_system_parameter_source_task_invalid",
    ...
    profiles = [
        ...
        if _local_name(child.tag) == ROBUSTNESS_METHOD_SYSTEM_PARAMETER_PERMUTATION
    ]
    if len(profiles) != 1:
        raise ResearchRobustnessError(
            "robustness_system_parameter_missing",
            "installed Retester project does not contain one System Parameter Permutation profile; configure/save it in SQX first",
        )
    target = profiles[0]
    ...
    if use not in {"true", "false"}:
        raise ResearchRobustnessError(
            ...
            "native System Parameter Permutation profile has an invalid use attribute",
        )
    if require_enabled and use != "true":
        raise ResearchRobustnessError(
            "robustness_compiled_task_invalid",
            "compiled System Parameter Permutation profile is not enabled",
        )
    ...
    if active_others:
        raise ResearchRobustnessError(
            ...
            "System Parameter Permutation isolation requires all other native cross-checks disabled: "
            + ", ".join(active_others),
        )

    settings = _exact_child(target, "Settings", "robustness_system_parameter_invalid")
    optim_periods = _text(settings, "OptimPeriods", "robustness_system_parameter_invalid")
    optim_exit_types = _text(settings, "OptimExitTypes", "robustness_system_parameter_invalid")
    max_tests = _text(settings, "MaxTests", "robustness_system_parameter_invalid")
    if optim_periods.lower() not in {"true", "false"}:
        raise ResearchRobustnessError(
            "robustness_system_parameter_invalid",
            "native OptimPeriods value is not boolean",
        )
    if optim_exit_types.lower() not in {"true", "false"}:
        raise ResearchRobustnessError(
            "robustness_system_parameter_invalid",
    ...
    if parsed_max_tests < 1:
        raise ResearchRobustnessError(
            "robustness_system_parameter_invalid",
            "native MaxTests value must be positive",
        )

    values = {
        "OptimPeriods": optim_periods,
        "OptimExitTypes": optim_exit_types,
        "MaxTests": max_tests,
    }
    return root, target, values


def _rewrite_task(task_bytes: bytes) -> tuple[bytes, dict[str, str], bool]:
...
def compile_system_parameter_permutation_project(project_bytes: bytes) -> tuple[bytes, dict[str, object]]:
    """Enable only the installed native System Parameter Permutation profile."""
    ...
    if not compiled_project:
        raise ResearchRobustnessError(
            ...
            "System Parameter Permutation compilation produced an empty native project snapshot",
        )
    try:
    ...
    if compiled_settings != settings:
        raise ResearchRobustnessError(
            "robustness_compiled_project_invalid",
            "compiled System Parameter Permutation settings changed unexpectedly",
        )
...
def _record_identity(payload: dict[str, object]) -> None:
    ...
    if payload.get("method") != ROBUSTNESS_METHOD_SYSTEM_PARAMETER_PERMUTATION:
...
def _read_record(store: FileResearchCustodyStore, record_ref: EvidenceRef) -> dict[str, object]:
    ...
    if payload.get("native_settings") != native_settings:
        raise ResearchRobustnessError("robustness_record_corrupt", "compiled System Parameter Permutation settings do not match custody")
...
def read_native_system_parameter_permutation_result(
...
def start_native_system_parameter_permutation(
    store: FileResearchCustodyStore,
    ...
    *,
    ...
    """Run installed SQX System Parameter Permutation on one Historical Result."""
    ...
    if historical.get("state") != "completed" or historical.get("execution_completed") is not True:
        raise ResearchRobustnessError(
            ...
            "System Parameter Permutation requires one completed native Historical Result",
        )
    ...
    compiled_project_bytes, plan = compile_system_parameter_permutation_project(source_project_bytes)
    ...
    if (
        ...
        raise ResearchRobustnessError(
            ...
            "native Retester gateway returned an invalid System Parameter Permutation receipt",
        )
    ...
    if result_info["archive_sha256"] == source_result_sha:
        raise ResearchRobustnessError(
            ...
            "native System Parameter Permutation execution did not produce a changed SQX result archive",
        )
