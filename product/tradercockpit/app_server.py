...
from tradercockpit.research_project_reviews import API_PATH as PROJECT_REVIEW_API_PATH, project_review_response
...
def make_handler(
    ...
    class Handler(SimpleHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802 - stdlib handler API
            ...
            if parsed.path == PROJECT_REVIEW_API_PATH:
                if not self._research_client_is_loopback():
                    self._reject_non_loopback_research_request()
                    return
                if parsed.query:
                    self._json(400, {"detail": "Project review accepts no query parameters."})
                    return
                payload = self._request_json()
                if payload is not None:
                    status, response = project_review_response(research_store, sqx_home, payload,
                        trusted_launcher_sha256=trusted_launcher_sha256)
                    self._json(status, response)
                return
