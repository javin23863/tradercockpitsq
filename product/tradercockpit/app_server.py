    ...
    try:
        if entity_id is not None:
            if worker_registry is not None:
                return 200, refresh_native_job(research_store, entity_id, worker_registry=worker_registry)
...
def research_native_job_write_response(
    ...
    if payload.get("action") == "stop-builder":
        if set(payload) != {"action", "entity_id"} or not isinstance(payload.get("entity_id"), str) or not payload["entity_id"]:
            return 400, {
                "error": "invalid_request",
                "reason_code": "native_job_action_invalid",
                "detail": "Native job stop requires action=stop-builder and the exact native job entity_id.",
            }
        try:
            return 200, stop_native_job(research_store, payload["entity_id"], worker_registry=worker_registry)
        except ResearchNativeJobError as exc:
            return 409, {"error": "invalid_state", "reason_code": exc.code, "detail": exc.detail}
        except ResearchCustodyError as exc:
            status, error = (404, "not_found") if exc.code == "current_pointer_missing" else (409, "invalid_state")
            return status, {"error": error, "reason_code": exc.code, "detail": exc.detail}
...
def make_handler(
    ...
    ``worker_registry`` owns the supervised native Builder workers for this process;
    the desktop passes one bound to its worker supervisor.
    """

    directory = str(web_root.resolve())
    if worker_registry is None:
        worker_registry = NativeWorkerRegistry()
...
def main(argv: list[str] | None = None) -> int:
    ...
    server = ThreadingHTTPServer(
        ...
        make_handler(
            ...
            research_store,
            worker_registry=NativeWorkerRegistry(workers.register),
        ),
    )
