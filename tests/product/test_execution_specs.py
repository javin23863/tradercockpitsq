...
class ExecutionSpecTests(unittest.TestCase):
    def strategy(self, period: int = 10) -> StrategySpecV1:
        return StrategySpecV1(
            ...
            semantics={
                "entry": {"operator": "greater-than", "left": "close", "period": period},
    ...
    def test_strategy_dataclass_is_directly_canonicalizable_after_freeze(self):
        strategy = self.strategy()
        from tradercockpit.domain import canonical_json_bytes

        encoded = canonical_json_bytes(strategy)
        self.assertIn(b'"semantic_schema":"tc.strategy.rules.v1"', encoded)
    ...
    def test_candidate_keeps_lineage_separate_from_strategy_identity(self):
        ...
        manual = CandidateSpecV1(resolved.ref, "manual", parent_strategy_ref=base.ref)
    ...
    def test_candidate_rejects_wrong_reference_kind(self):
        data = DataSpecV1("ES", "1m", "fixture", "rev-1", "America/Chicago", "CME", "2025-01-01T00:00:00Z", "2025-01-02T00:00:00Z", "none")
    ...
    def test_data_timestamps_are_timezone_canonical_and_identity_equivalent(self):
        zulu = DataSpecV1("ES", "7m", "fixture", "rev-1", "America/Chicago", "CME", "2025-01-01T00:00:00Z", "2025-01-02T00:00:00Z", "none")
        offset = DataSpecV1("ES", "7m", "fixture", "rev-1", "America/Chicago", "CME", "2024-12-31T19:00:00-05:00", "2025-01-01T19:00:00-05:00", "none")
    ...
    def test_data_refuses_naive_or_reversed_window(self):
        ...
        with self.assertRaisesRegex(SpecValidationError, "explicit timezone"):
            DataSpecV1(*args, "2025-01-01T00:00:00", "2025-01-02T00:00:00Z", "none")
        with self.assertRaisesRegex(SpecValidationError, "start must be before end"):
            DataSpecV1(*args, "2025-01-03T00:00:00Z", "2025-01-02T00:00:00Z", "none")

    def test_data_window_orders_microseconds_correctly(self):
        args = ("ES", "1m", "fixture", "rev-1", "America/Chicago", "CME")
        valid = DataSpecV1(*args, "2025-01-01T00:00:00.100000Z", "2025-01-01T00:00:00.200000Z", "none")
        self.assertLess(valid.start, valid.end)
        with self.assertRaisesRegex(SpecValidationError, "start must be before end"):
            DataSpecV1(*args, "2025-01-01T00:00:00.200000Z", "2025-01-01T00:00:00.100000Z", "none")

    def test_execution_models_are_explicit_and_unique(self):
        execution = self.execution()
        self.assertEqual(tuple(model.kind for model in execution.models), ("fees", "fill", "slippage"))
        with self.assertRaisesRegex(SpecValidationError, "duplicate execution model kind"):
            ExecutionSpecV1(
                Decimal("1000"),
                "USD",
                (ExecutionModelV1("fees", "none", {}), ExecutionModelV1("fees", "flat", {"amount": Decimal("1")})),
            )

    def test_execution_model_order_does_not_change_identity(self):
        left = ExecutionSpecV1(
            Decimal("1000"),
            "USD",
            (ExecutionModelV1("fill", "bar-close", {}), ExecutionModelV1("fees", "none", {})),
        )
        right = ExecutionSpecV1(
            Decimal("1000"),
            "USD",
            (ExecutionModelV1("fees", "none", {}), ExecutionModelV1("fill", "bar-close", {})),
        )
        self.assertEqual(left.ref, right.ref)
        self.assertEqual(tuple(model.kind for model in left.models), ("fees", "fill"))

    def test_execution_refuses_float_cash_and_float_model_parameters(self):
        with self.assertRaises(SpecValidationError):
            ExecutionSpecV1(1000.0, "USD", (ExecutionModelV1("fill", "bar-close", {}),))  # type: ignore[arg-type]
        with self.assertRaisesRegex(SpecValidationError, "float is not permitted"):
    ...
    def test_backtest_run_binds_all_custody_refs_and_seed(self):
        ...
        data = DataSpecV1("ES", "1m", "fixture", "rev-1", "America/Chicago", "CME", "2025-01-01T00:00:00Z", "2025-02-01T00:00:00Z", "none")
        execution = self.execution()
        engine = EngineBuildSpecV1("tradercockpit", "abc123", "a" * 64)
        run = BacktestRunSpecV1(candidate.ref, data.ref, execution.ref, engine.ref, random_seed=7)
        same = BacktestRunSpecV1(candidate.ref, data.ref, execution.ref, engine.ref, random_seed=7)
        other_seed = BacktestRunSpecV1(candidate.ref, data.ref, execution.ref, engine.ref, random_seed=8)
    ...
    def test_backtest_run_rejects_cross_kind_substitution(self):
        ...
        with self.assertRaisesRegex(SpecValidationError, "random_seed"):
            BacktestRunSpecV1(candidate.ref, data.ref, execution.ref, engine.ref, random_seed=-1)
