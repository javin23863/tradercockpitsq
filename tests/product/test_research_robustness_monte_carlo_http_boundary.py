...
class ResearchMonteCarloHttpBoundaryTests(unittest.TestCase):
    ...
    def test_start_forwards_only_exact_historical_result_identity(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            try:
                with patch(
                    "tradercockpit.research_retester_http.start_native_monte_carlo_manipulation",
                    return_value=result,
                ) as starter:
                    for forbidden in (
                        {"NumberOfSimulations": 30},
                        {"RandomizeTradesOrder": "resampling"},
                        {"RandomlySkipTrades": 10},
                        {"acceptance": {"NetProfit": 60}},
                        {"task": 2},
    ...
    def test_read_uses_method_specific_exact_validation_ref(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            try:
                with patch(
                    "tradercockpit.research_retester_http.read_native_monte_carlo_manipulation_result",
                    return_value=result,
                ) as reader:
                    status, payload = self._post(
                        endpoint,
                        {"action": "read-monte-carlo-manipulation", "validation_ref": self.VALIDATION_REF},
                    )
                    ...
                    status, payload = self._post(
                        endpoint,
                        {"action": "read-monte-carlo-manipulation", "validation_ref": self.VALIDATION_REF, "latest": True},
                    )
                    self.assertEqual(status, 400)
