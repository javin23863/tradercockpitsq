package SQ.Blocks.Indicators.Aroon;

import SQ.Internal.ConditionBlock;
import com.strategyquant.datalib.TradingException;
import com.strategyquant.tradinglib.BuildingBlock;
import com.strategyquant.tradinglib.ChartData;
import com.strategyquant.tradinglib.Help;
import com.strategyquant.tradinglib.OppositeBlock;
import com.strategyquant.tradinglib.Parameter;
import com.strategyquant.tradinglib.ParameterSet;
import com.strategyquant.tradinglib.blocks.annotations.ParameterSets;

@BuildingBlock(name = "Aroon Up crosses above Aroon Down", display = "Aroon(@Chart@#Period#).Up[#Shift#] crosses above Aroon Down", returnType = 3)
@Help("Is triggered if Aroon Up crosses above Aroon Down")
@OppositeBlock("AroonCrossesBelow")
@ParameterSets(
   {
         @ParameterSet(set = "Period=14"),
         @ParameterSet(set = "Period=20"),
         @ParameterSet(set = "Period=30"),
         @ParameterSet(set = "Period=40"),
         @ParameterSet(set = "Period=50")
   }
)
public class AroonCrossesAbove extends ConditionBlock {
   @Parameter
   public ChartData Input;
   @Parameter(category = "Default", name = "Period", minValue = 0.0, maxValue = 1000.0, defaultValue = "14", step = 1.0)
   public int Period;
   @Parameter
   public int Shift;

   @Override
   public boolean OnBlockEvaluate() throws TradingException {
      Aroon var1 = this.Strategy.Indicators.Aroon(this.Input, this.Period);
      double var2 = var1.Down.getRounded(this.Shift + 1);
      double var4 = var1.Down.getRounded(this.Shift);
      double var6 = var1.Up.getRounded(this.Shift + 1);
      double var8 = var1.Up.getRounded(this.Shift);
      return var6 < var2 && var8 > var4;
   }
}
