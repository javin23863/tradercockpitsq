...
@BuildingBlock(name = "(VP24) Value Shift Down (Bear Regime)", display = "ValueShiftDown(@Chart@#LookbackBars#,#MinimumGap#)", returnType = 3)
@Help(
   "Strong bearish regime: current session's VAH sits entirely below the prior session's VAL. The entire value area has shifted lower, indicating aggressive acceptance of lower prices. Prefer short setups—sell rallies into the new value area."
)
...
public class VPValueShiftDown extends ConditionBlock {
   ...
   private boolean _evaluateSignal() throws TradingException {
      ...
      double var4 = var3.VAH.getRounded(this.Shift);
      double var6 = var3.VAL.getRounded(this.Shift + this.LookbackBars);
