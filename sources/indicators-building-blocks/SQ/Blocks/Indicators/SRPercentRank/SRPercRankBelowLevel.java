...
@BuildingBlock(
   name = "(PRBL) SR Percent Rank is below Level",
   display = " SR Percent Rank(@Chart@#Mode#,#Length#,#ATRPeriod#)[#Shift#] is below #Level#",
   returnType = 3
)
@Help("Is triggered if SRPercentRank is below Level")
@OppositeBlock("SRPercRankBelowLevel")
@ParameterSets(
   {
...
public class SRPercRankBelowLevel extends ConditionBlock {
   ...
   @Parameter(defaultValue = "20", isPeriod = true, minValue = 0.5, maxValue = 100.0, step = 0.5)
   public double Level;
   @Parameter
   public int Shift;
