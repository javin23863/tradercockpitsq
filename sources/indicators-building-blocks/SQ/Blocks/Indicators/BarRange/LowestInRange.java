...
@BuildingBlock(name = "Lowest in range", display = "LowestInRange(@Chart@#TimeFrom#, #TimeTo#)[#Shift#]", returnType = 2)
@Help("returns lowest low of candles in given time range")
@Indicator
@OppositeBlock("HighestInRange")
public class LowestInRange extends IndicatorBlock {
   @Parameter
   ...
   private double lowestValue = 0.0;
   ...
   protected void OnBarUpdate() throws TradingException {
      ...
      if (var5 >= this.timeTo) {
         long var3 = SQTime.correctDayStart(this.Input.Time(0));
         ...
         this.lastValue = this.lowestValue;
         ...
         if (this.timeFrom <= var5) {
            this.lowestValue = this.Input.Low(0);
         }
      } else if (var5 >= this.timeFrom) {
         this.lowestValue = Math.min(this.lowestValue, this.Input.Low(0));
      } else {
         this.lowestValue = Double.MAX_VALUE;
      }

      if (this.lastValue < Double.MAX_VALUE) {
