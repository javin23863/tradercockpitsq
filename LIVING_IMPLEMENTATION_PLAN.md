...
- producer-backed native Backtest Robustness methods using the installed Retester project as executable configuration authority (Higher Precision, Additional Markets, Monte Carlo retest, Walk-Forward, Walk-Forward Matrix, What-If, System Parameter Permutation, Monte Carlo manipulation, Sequential Optimization), isolated exact-baseline staging, the trusted Retester task-1 gateway, durable prepared/completed/failed/interrupted Proof custody, exact catalog/readback, and the canonical desktop Robustness surface; Sequential stays unavailable until the installed profile has Settings; Evidence Proof accepts Higher Precision and Additional Markets only;
...
- [x] Inspect/run the actual SQX downstream validation/Robustness workflow sufficiently to connect the installed Retester CrossChecks on the canonical contracts. Connected methods are Higher Precision, Additional Markets, Monte Carlo retest, Walk-Forward, Walk-Forward Matrix, What-If, System Parameter Permutation, Monte Carlo manipulation, and Sequential Optimization. Sequential stays unavailable until the installed profile has Settings. Evidence Proof accepts Higher Precision or Additional Markets. `WF*` / confidence-level Monte Carlo stay `unevaluated` until a native Walk-Forward / CL result key exists; unused Main/HP SQStats default zeros are not a Walk-Forward run. Installed 144.2953 `.sqx` archives stamp `version.txt=1`; that format stamp maps to product build `144.2953`. Builder launch stages the Task-rooted `.cfx` sqcli loadconfig accepts, fails closed on `Cannot load config` / `Cannot start project` at exit 0, and treats `Project started` as the start receipt so the in-process Builder run is not waited out. Isolated Retester projects start with `action=start` (sqcli `startOnlyTask task=1` skips XML `name="Retest"`) and wait up to 1800s for the CrossCheck to finish; the 60s loadconfig cap is not the Retester run cap. A CrossCheck is not complete unless that method's producer result key exists.
- [x] Backtest Robustness shows producer-backed CrossCheck execution/custody state only; no TraderCockpit validation algorithm or reconstructed Walk-Forward / Monte Carlo engine substitutes for the native producer. Pass/fail for a method is SQX vs cockpit parity on the same symbol and the same native action.
...
**Current Research lane:** expand the canonical backend/native read models and practical Research UI beyond the currently mapped surface, starting with producer-backed Random Discovery vs Genetic Evolution depth, rule/block families, parameter representations, selection/ranking, and deeper Custom Project controls/readback as those seams are directly observed. Connected CrossChecks launch through the installed Retester profiles; Sequential still needs a saved SQX Settings profile, and `WF*` / CL Monte Carlo columns become pass/fail only after a native run writes them. Keep the versioned coverage inventory synchronized as each producer capability becomes exposed; do not mark the coverage-proof or integrated-desktop-review gates complete merely because today’s 12 read-model capabilities are mapped.
