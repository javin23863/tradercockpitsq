angular.module('app.resultsdatabankactions.movetopm', ['sqplugin']).config(function(sqPluginProvider) {
    
    sqPluginProvider.plugin("ResultsDatabankAction", 300, {
        title: Ltsq("Portfolio")+":"+Ltsq("Move to Portfolio Master"),
        class: 'btn btn-normal btn-default',
        controller: btnCtrl,
        product: "BUILDER,RETESTER,OPTIMIZER,TASKMANAGER",
        id: "databank-action-move-to-pm"
    });
    ...
    function btnCtrl($rootScope, $scope, DatabankActionsService, L, SQConstants, DatabankService, AppService){
        ...
        $scope.onClick = function(){
            if($rootScope.project.state != projectStates.loading && $rootScope.project.state != projectStates.running) {
                ...
                $rootScope.showConfirm("Move to Portfolio Master",  "Are you sure you want to move selected strategies to Portfolio Master?",  function() {
                    DatabankService.moveToPortfolioMaster(strategies).then(function(result) {
                        if(!result.success){
                            $rootScope.showError(L.tsq("Error while moving strategies to Portfolio Master. %s", [ result.error ]));
                        }
                    });
                });
