...
public class GenEvoFitnessISV extends EngineChart {
	...
	public GenEvoFitnessISV() {
		super(L.tsq("Island #1 - Fitness IS Validation"), TYPE_PROJECT_BASED);
		...
		this.bestSeries = new TimeSeries(L.tsq("Top Strategy"), TimeSeriesLineChart.PointsCount);
		this.bestSeries.color = ChartsConst.COLOR_GREEN;
		this.chart.addSeries(bestSeries);
		
		this.best5AvgSeries = new TimeSeries(L.tsq("Top Ten Avg"), TimeSeriesLineChart.PointsCount);
		this.best5AvgSeries.color = ChartsConst.COLOR_BLUE;
		this.chart.addSeries(best5AvgSeries);
		
		this.avgSeries = new TimeSeries(L.tsq("All Avg"), TimeSeriesLineChart.PointsCount);
			...
			if(islandsEvoData != null && islandsEvoData.length > 0) {
				...
				if(islandEvoData.generationData.length > islandEvoData.generation && islandEvoData.generationData[islandEvoData.generation] != null) { 
					...
					if(generationData != null) {
						bestFitness = generationData.bestFitnessISV;
						best5Fitness = generationData.topAvgFitnessISV;
						avgFitness = generationData.avgFitnessISV;
					}
				}
			}
		}
