...
@BuildingBlock(display="BarsSinceOrderClosed(#Symbol#, #MagicNumber#, #Direction#, \"#Comment#\")", returnType = ReturnTypes.Number)
...
public class BarsSinceOrderClosed extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		Order order = OrderFunctions.findLastHistoryOrder(Strategy, Symbol, Direction, MagicNumber, Comment);
		if(order != null) {
			long orderOpenTime = order.CloseTime;
			int numberOfBars = 0;

			for(int i=0; i<10000; i++) {
				if(orderOpenTime < Strategy.Time(i)) {
					numberOfBars++;
				}
			}
