...
@BuildingBlock(display="Market Position(#Symbol#, #MagicNumber#, \"#Comment#\") is Not Long", returnType = ReturnTypes.Boolean)
	...
	public boolean OnBlockEvaluate() throws TradingException {
		for(int i=0; i<Strategy.Trader.getOpenOrdersCount(true); i++) {
			...
			if(OrderFunctions.identify(order, Strategy, Symbol, 0, MagicNumber, Comment)) {
				if(order.getDirection() == Directions.Long) return false;
			}
		}
