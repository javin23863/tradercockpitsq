...
@BuildingBlock(name="(WFU) Woodies Famir UP", display="Woodies Famir UP(@Chart@#Period#,#Factor#)[#Shift#]", returnType = ReturnTypes.Boolean)
@Help("Is triggered if CCI makes ZLR Down pattern and next bar fails. It is counter trend pattern; Factor = minimum distance between top and bottom hook on CCI chart.")
@OppositeBlock("WoodiesFamirDown")
@ParameterSet(set="Period=14,Factor = 5")
@ParameterSet(set="Period=12,Factor = 5")
@ParameterSet(set="Period=24,Factor = 5")
public class WoodiesFamirUP extends ConditionBlock {
...
@Parameter
	...
	public boolean OnBlockEvaluate() throws TradingException {
		...
		boolean brokenHookUP = (cci1<cci2)&&(cci3<cci2) && (currCCI>cci1); 
		boolean factorCondition = ((cci2-cci1)> Factor && (cci2-cci3)>Factor);

		return (currCCI>-50 &&cci1<0 && cci2<0 && cci3<0 && cci4<0 && cci5<0 && cci6<0) && brokenHookUP == true && factorCondition == true ;
	}
