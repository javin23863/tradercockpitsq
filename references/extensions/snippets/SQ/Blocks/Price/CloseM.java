...
@BuildingBlock(name="(MC) Monthly Close", returnType=ReturnTypes.Price, display="CloseM[@Chart@#Shift#]")
@SortOrder(800)
public class CloseM extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {			
			...
			return Strategy.Stockpicker.data.CloseM(chartIndex, relativeShift + Shift);
		}
		...
		if(Engines.isTradestationEngine(Strategy.getEngine())) {
			...
			if (Shift == 0 && monthsOffset == 0 && relativeShift != 0) {
				...
				if (barsShiftedSeries.get(relativeShift) == 0) {
					double closeM =  Chart.Close.get(barIndex);
					barsShiftedSeries.set(relativeShift, closeM);
		            return closeM;
				}
				return barsShiftedSeries.get(relativeShift);
			}else {
				return Chart.CloseM(Shift + monthsOffset);
			}
		}
		else {
			return Chart.CloseM(Shift + relativeShift);
		}
	}
}