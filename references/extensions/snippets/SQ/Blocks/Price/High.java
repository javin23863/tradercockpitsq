...
@BuildingBlock(name="(H) High", returnType=ReturnTypes.Price, display="High[@Chart@#Shift#]")
@SortOrder(600)
@OppositeBlock("Low")
public class High extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {			
			...
			return Strategy.Stockpicker.data.High(chartIndex, relativeShift + Shift);
		}
		
		return Chart.High(relativeShift + Shift);
	}
