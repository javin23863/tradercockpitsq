...
@BuildingBlock(name="(V) Volume", returnType=ReturnTypes.Number, display="Volume[@Chart@#Shift#]")
@IgnoreInBuilder
public class Volume extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {		
			...
			return Strategy.Stockpicker.data.Volume(chartIndex, relativeShift + Shift);
		}
		
		return Chart.Volume(relativeShift + Shift);
	}
