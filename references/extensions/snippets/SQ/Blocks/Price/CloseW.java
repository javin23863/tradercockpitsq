...
@BuildingBlock(name="(WC) Weekly Close", returnType=ReturnTypes.Price, display="CloseW[@Chart@#Shift#]")
@SortOrder(800)
public class CloseW extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {			
			...
			return Strategy.Stockpicker.data.CloseW(chartIndex, relativeShift + Shift);
		}
		...
		if(Engines.isTradestationEngine(Strategy.getEngine())) {
			...
			if (Shift == 0 && weeksOffset == 0 && relativeShift != 0) {
				...
				if (barsShiftedSeries.get(relativeShift) == 0) {
					double closeW =  Chart.Close.get(barIndex);
					barsShiftedSeries.set(relativeShift, closeW);
		            return closeW;
				}
				return barsShiftedSeries.get(relativeShift);
			}else {
				return Chart.CloseW(Shift + weeksOffset);
			}
		}
		else {
			return Chart.CloseW(Shift + relativeShift);
		}
	}
}