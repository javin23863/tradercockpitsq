...
@BuildingBlock(name="(WH) Weekly High", returnType=ReturnTypes.Price, display="HighW[@Chart@#Shift#]")
@SortOrder(600)
@OppositeBlock("LowW")
public class HighW extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {			
			if(Strategy.Stockpicker.strategyTriggeredAt() == PickerTriggerTypes.OnBarOpen && (relativeShift + Shift == 0)) {
				return Strategy.Stockpicker.data.OpenW(chartIndex, relativeShift + Shift);
			}
			
			return Strategy.Stockpicker.data.HighW(chartIndex, relativeShift + Shift);
		}
		
		//In EasyLanguage, OHLC are shifted by bars
		if(Engines.isTradestationEngine(Strategy.getEngine())) {
			int weeksOffset = TimeDiffUtil.getDiffInWeeks(Chart, relativeShift);
			
			if (Shift == 0 && weeksOffset == 0 && relativeShift != 0) {
				...
				if (barsShiftedSeries.get(relativeShift) == 0) {
					...
					double highW = calculateHighDUpToBar(barIndex, maxIndex);
					barsShiftedSeries.set(relativeShift, highW);
		            return highW;
				}
				return barsShiftedSeries.get(relativeShift);
			}else {
				return Chart.HighW(Shift + weeksOffset);
			}
		}
		else {
			return Chart.HighW(Shift + relativeShift);
		}
	}
    ...
    private double calculateHighDUpToBar(int barIndex, int maxIndex) throws TradingException {
        ...
        for (int i = barIndex; i <= maxIndex; i++) {
        	if (SQTime.isSameWeek(Chart.Time.get(i), Chart.Time.get(barIndex)) == 2) {
