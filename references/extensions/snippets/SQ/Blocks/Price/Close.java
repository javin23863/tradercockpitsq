...
@BuildingBlock(name="(C) Close", returnType=ReturnTypes.Price, display="Close[@Chart@#Shift#]")
@SortOrder(800)
public class Close extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {			
			...
			return Strategy.Stockpicker.data.Close(chartIndex, relativeShift + Shift);
		}
		
		return Chart.Close(relativeShift + Shift);
	}
