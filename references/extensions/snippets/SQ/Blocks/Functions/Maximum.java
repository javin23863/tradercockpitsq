...
@BuildingBlock(name="(MAX) Maximum", display="Max(#Value1#, #Value2#)", returnType = ReturnTypes.PriceOrNumber)
...
public class Maximum extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		return Math.max(Value1.evaluateBlock(relativeShift), Value2.evaluateBlock(relativeShift));
	}
