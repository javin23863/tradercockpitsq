...
public class OpenDrawdownPct extends DatabankColumn {
	...
	public OpenDrawdownPct() {
		super(L.tsq("Open Drawdown %"), DatabankColumn.Decimal2Pct, ValueTypes.Minimize, 0, 0, 10000);
		...
		setTooltip(L.tsq("Max % Drawdown computed from MAE (Maximum Adverse Excursion)"));
	}
	...
	public double compute(SQStats stats, StatsTypeCombination combination, OrdersList ordersList, SettingsMap settings, SQStats statsLong, SQStats statsShort) throws Exception {
		...
		for(int i=0; i<ordersList.size(); i++) {
            ...
            openDD = specialSubtraction(peakEquity, maeEquity);
            
            // percentage DD is computed as % of current DD in money vs equity balance at the peak
           	o.Extra1 = (float) getPercentageDD(openDD, peakEquity); 
