...
{
   long bg_l = 0;
   if(!ChartGetInteger(0, CHART_COLOR_BACKGROUND, 0, bg_l))
      return false; // assume dark on error
...
void OnChartEvent(const int id,
   ...
   if(id == CHARTEVENT_CHART_CHANGE) {
      ...
      if(sparam == "sqBtnLong") {
         ...
         ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_STATE, false);
         ChartRedraw();
      }
      else if(sparam == "sqBtnShort") {
         ShortEnabled = !ShortEnabled;
         sqUpdateDirectionButtons();
         ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_STATE, false);
         ChartRedraw();
      }
   }
}
...
int SqCornerToMT5(ENUM_SQ_CORNER c)
{
   switch(c)
   {
...
int SqAnchorToMT5(ENUM_SQ_CORNER c)
{
   switch(c)
   {
      case SQ_LEFT_UPPER:  return ANCHOR_LEFT_UPPER;
      case SQ_RIGHT_UPPER: return ANCHOR_RIGHT_UPPER;
      case SQ_LEFT_LOWER:  return ANCHOR_LEFT_LOWER;
      case SQ_RIGHT_LOWER: return ANCHOR_RIGHT_LOWER;
   }
...
string correctSymbol(string symbol){
    if(symbol == NULL || symbol == "NULL" || symbol == "Current" || symbol == "0" || symbol == "Same as main chart" || symbol == "Main chart symbol") {
        return Symbol();
    }
    <@mt5PrintSubchartConditions />
    else return symbol;
}
...
ENUM_ORDER_TYPE_FILLING GetFilling(const string Symb, const  ENUM_TRADE_REQUEST_ACTIONS action, const ENUM_ORDER_TYPE_FILLING Type = ORDER_FILLING_FOK) {
   ...
   if(FillingMode == 0 || (Type >= ORDER_FILLING_RETURN) || ((FillingMode & (Type + 1)) != Type + 1)) {
      ...
      if (((ExeMode == SYMBOL_TRADE_EXECUTION_EXCHANGE) || (ExeMode == SYMBOL_TRADE_EXECUTION_INSTANT)) 
            && (action!=TRADE_ACTION_DEAL && action!=TRADE_ACTION_CLOSE_BY && action!=TRADE_ACTION_REMOVE)) {  // and NOT Market
      <#else>
      if((ExeMode == SYMBOL_TRADE_EXECUTION_EXCHANGE) || (ExeMode == SYMBOL_TRADE_EXECUTION_INSTANT)) {
      </#if>
...
void sqInitInfoPanel() {
      ...
      sqLabelCorner        = SqCornerToMT5(InpInfoPanelCorner);
      int anchor               = SqAnchorToMT5(InpInfoPanelCorner);
      ObjectCreate(ChartID(), "line1", OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(ChartID(), "line1", OBJPROP_CORNER, sqLabelCorner);
      ObjectSetInteger(ChartID(), "line1", OBJPROP_ANCHOR,    anchor);
      ...
      ObjectSetInteger(ChartID(), "line1", OBJPROP_COLOR, GetAdaptiveTextColor());
      setupLabel("line1", sqStrategyName + " | Magic: " + (string)MagicNumber, "Tahoma", 9);

      ObjectCreate(ChartID(), "linec", OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(ChartID(), "linec", OBJPROP_CORNER, sqLabelCorner);
      ObjectSetInteger(ChartID(), "linec", OBJPROP_ANCHOR,    anchor);
      ...
      ObjectSetInteger(ChartID(), "linec", OBJPROP_COLOR, GetAdaptiveTextColor());
      ...
      ObjectSetInteger(ChartID(), "line2", OBJPROP_ANCHOR,    anchor);
      ...
      ObjectSetInteger(ChartID(), "line2", OBJPROP_COLOR, GetAdaptiveTextColor());
      setupLabel("line2", "------------------------------------------");

      ObjectCreate(ChartID(), "lines", OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(ChartID(), "lines", OBJPROP_CORNER, sqLabelCorner);
      ObjectSetInteger(ChartID(), "lines", OBJPROP_ANCHOR,    anchor);
      ...
      ObjectSetInteger(ChartID(), "lines", OBJPROP_COLOR, GetAdaptiveTextColor());
      ...
      ObjectSetInteger(ChartID(), "lineopl", OBJPROP_ANCHOR,    anchor);
      ...
      ObjectSetInteger(ChartID(), "lineopl", OBJPROP_COLOR, GetAdaptiveTextColor());
      ...
      ObjectSetInteger(ChartID(), "linea", OBJPROP_ANCHOR,    anchor);
      ...
      ObjectSetInteger(ChartID(), "linea", OBJPROP_COLOR, GetAdaptiveTextColor());
      ...
      ObjectSetInteger(ChartID(), "lineto", OBJPROP_ANCHOR,    anchor);
      ...
      ObjectSetInteger(ChartID(), "lineto", OBJPROP_COLOR, GetAdaptiveTextColor());
      ...
      ObjectSetInteger(ChartID(), "linetp", OBJPROP_ANCHOR,    anchor);
      ...
      ObjectSetInteger(ChartID(), "linetp", OBJPROP_COLOR, GetAdaptiveTextColor());
...
void sqCreateDirectionButtons() {
   ...
   if(isBottomCorner) {
      ...
      ObjectSetInteger(ChartID(), "line1",   OBJPROP_YDISTANCE, sqOffsetVertical + 0   + panelShift);
      ObjectSetInteger(ChartID(), "linec",   OBJPROP_YDISTANCE, sqOffsetVertical + 16  + panelShift);
      ObjectSetInteger(ChartID(), "line2",   OBJPROP_YDISTANCE, sqOffsetVertical + 28  + panelShift);
      ObjectSetInteger(ChartID(), "lines",   OBJPROP_YDISTANCE, sqOffsetVertical + 44  + panelShift);
      ObjectSetInteger(ChartID(), "lineopl", OBJPROP_YDISTANCE, sqOffsetVertical + 60  + panelShift);
      ObjectSetInteger(ChartID(), "linea",   OBJPROP_YDISTANCE, sqOffsetVertical + 76  + panelShift);
      ObjectSetInteger(ChartID(), "lineto",  OBJPROP_YDISTANCE, sqOffsetVertical + 92  + panelShift);
      ObjectSetInteger(ChartID(), "linetp",  OBJPROP_YDISTANCE, sqOffsetVertical + 108 + panelShift);
   } else {
   ...
   // Long button
   ObjectCreate(ChartID(), "sqBtnLong", OBJ_BUTTON, 0, 0, 0);
   ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_XDISTANCE, btnLongX);
   ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_YDISTANCE, btnY);
   ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_XSIZE, btnWidth);
   ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_YSIZE, btnHeight);
   ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_CORNER, sqLabelCorner);
   ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_FONTSIZE, 8);
   ObjectSetString(ChartID(), "sqBtnLong", OBJPROP_FONT, "Tahoma");
   ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_SELECTABLE, false);

   // Short button
   ObjectCreate(ChartID(), "sqBtnShort", OBJ_BUTTON, 0, 0, 0);
   ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_XDISTANCE, btnShortX);
   ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_YDISTANCE, btnY);
   ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_XSIZE, btnWidth);
   ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_YSIZE, btnHeight);
   ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_CORNER, sqLabelCorner);
   ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_FONTSIZE, 8);
   ObjectSetString(ChartID(), "sqBtnShort", OBJPROP_FONT, "Tahoma");
   ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_SELECTABLE, false);
...
void sqUpdateDirectionButtons() {
   // Long button
   if(LongEnabled) {
      ObjectSetString(ChartID(), "sqBtnLong", OBJPROP_TEXT, "Long Enabled");
      ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_BGCOLOR, clrForestGreen);
      ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_COLOR, clrWhite);
   } else {
      ObjectSetString(ChartID(), "sqBtnLong", OBJPROP_TEXT, "Long Disabled");
      ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_BGCOLOR, clrFireBrick);
      ObjectSetInteger(ChartID(), "sqBtnLong", OBJPROP_COLOR, clrWhite);
   }

   // Short button
   if(ShortEnabled) {
      ObjectSetString(ChartID(), "sqBtnShort", OBJPROP_TEXT, "Short Enabled");
      ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_BGCOLOR, clrForestGreen);
      ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_COLOR, clrWhite);
   } else {
      ObjectSetString(ChartID(), "sqBtnShort", OBJPROP_TEXT, "Short Disabled");
      ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_BGCOLOR, clrFireBrick);
      ObjectSetInteger(ChartID(), "sqBtnShort", OBJPROP_COLOR, clrWhite);
   }
}
...
void sqDeinitInfoPanel() {
   ...
   ObjectDelete(ChartID(), "sqBtnLong");
   ObjectDelete(ChartID(), "sqBtnShort");
}
...
void setupLabel(string objectName, string text, const string font = "Tahoma", const int fontSize = 8, const color objColor = NULL){
   ...
   ObjectSetInteger(ChartID(), objectName, OBJPROP_COLOR, GetAdaptiveTextColor());
}
...
bool IsMarketOpen(const string symbol, const bool debug = false)
{
   ...
   // Day of week in server time
   MqlDateTime dt;
   TimeToStruct(serverTime, dt);
   const ENUM_DAY_OF_WEEK day_of_week = (ENUM_DAY_OF_WEEK)dt.day_of_week;

   // Time-of-day in seconds (0..86399)
   const int secondsInDay = (int)PeriodSeconds(PERIOD_D1);
   const int time = (int)(serverTime % secondsInDay);
   ...
   for (int session = 0; ; session++)
   {
      ...
      if (!SymbolInfoSessionTrade(symbol, day_of_week, session, from, to))
      {
         ...
         if (err == ERR_MARKET_SESSION_INDEX)
         {
         ...
         if (debug && err != 0)
            PrintFormat("%s(%s): SymbolInfoSessionTrade error %d",
                        __FUNCTION__, symbol, err);
         break;
      }

      // Convert session times to time-of-day
      const int sessionFrom = (int)(from % secondsInDay);
      const int sessionTo   = (int)(to   % secondsInDay);

      if (debug)
         PrintFormat("%s(%s): Checking %d >= %d && %d <= %d (session %d)",
                     __FUNCTION__, symbol,
                     time, sessionFrom, time, sessionTo, session);

      // Normal session (no midnight wrap)
      if (sessionFrom < sessionTo)
      {
         if (time >= sessionFrom && time <= sessionTo)
         {
            if (debug)
               PrintFormat("%s Market is open (session %d)", __FUNCTION__, session);
            return true;
         }
      }
      // Overnight session that crosses midnight
      else
      {
         if (time >= sessionFrom || time <= sessionTo)
         {
            if (debug)
               PrintFormat("%s Market is open (overnight session %d)", __FUNCTION__, session);
            return true;
         }
      }
   }
