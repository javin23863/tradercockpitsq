...
public class DrawdownPips extends DatabankColumn {
	...
	public DrawdownPips() {
		super(L.tsq("Max DD pips"), DatabankColumn.Decimal2Pips, ValueTypes.Minimize, 0, 0, 1000);

		setTooltip(L.tsq("Max Drawdown in pips"));
	...
	public double compute(SQStats stats, StatsTypeCombination combination, OrdersList ordersList, SettingsMap settings, SQStats statsLong, SQStats statsShort) throws Exception {
		...
		for(int i=0; i<ordersList.size(); i++) {
            ...
            o.PipsDD =  (float) specialSubtraction(peakPips, equityPips);
            
            if(equityPips > peakPips) {
            	peakPips = equityPips;
            }
    	}		

		double maxPipsDD = 99999999;
		...
		for(int i=0; i<ordersList.size(); i++) {
			...
			maxPipsDD = Math.min(maxPipsDD, o.PipsDD);
		}
		...
		return round2(Math.abs(maxPipsDD));
	}
