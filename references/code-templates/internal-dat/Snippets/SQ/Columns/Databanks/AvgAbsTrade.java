...
public class AvgAbsTrade extends DatabankColumn {
	...
	public AvgAbsTrade() {
		super(L.tsq("Avg. Abs Trade"), DatabankColumn.Decimal2PL, ValueTypes.Maximize, 0, 0, 200);
		
		setDependencies("NumberOfTrades");
		setTooltip(L.tsq("Average Absolute Trade (including losing trades)"));
	}
	...
	public double compute(SQStats stats, StatsTypeCombination combination, OrdersList ordersList, SettingsMap settings, SQStats statsLong, SQStats statsShort) throws Exception {
		...
		for(int i = 0; i<ordersList.size(); i++) {
			...
			sumTrades += Math.abs(PL);
		}

		int numberOfTrades = stats.getInt("NumberOfTrades");
		
		return SQUtils.round(safeDivide(sumTrades, numberOfTrades), 4);
	}		
}