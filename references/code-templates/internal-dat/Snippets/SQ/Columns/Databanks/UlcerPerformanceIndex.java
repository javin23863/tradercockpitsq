...
public class UlcerPerformanceIndex extends DatabankColumn {
	public static final Logger Log = LoggerFactory.getLogger("UlcerPerformanceIndex");
	...
	public UlcerPerformanceIndex() {
		...
		this.setTooltip(L.tsq("Ulcer Performance Index (annualized)"));

		// restrict UlcerPerformanceIndex computation only for money PL Type (we'll not compute separate UlcerPerformanceIndex for % or pips results, it is as same as for money)
		setPLTypeRestrictions(PlTypes.Money); 		

		// restrict UlcerPerformanceIndex computation only for Both direction, we'll not compute Sharpe for Long only or Short only results
		setDirectionRestrictions(Directions.Both); 	
		setDependencies("UlcerIndex");		
	}
	...
	public double compute(SQStats stats, StatsTypeCombination combination, OrdersList ordersList, SettingsMap settings, SQStats statsLong, SQStats statsShort) throws Exception {
		...
		double ulcerIndex = stats.getDouble("UlcerIndex");
		double UPI = Math.sqrt(252) * SQUtils.safeDivide(returnsMean, ulcerIndex);
