...
import SQ.Columns.Databanks.Note;

public class Note extends DatabankColumn {
    
	public Note() {
		super(L.tsq("Note"), DatabankColumn.Text, ValueTypes.Minimize, 0, 0, 0);
		setWidth(150);
		setEditable(true);
	}
	...
	public String getValue(ResultsGroup results, String resultKey, byte direction, byte plType, byte sampleType) throws Exception {
		String note = results.specialValues().getString(SpecialValues.Note, "");
		
		return "{{inputWidget value='"+note+"'}}";
	}
	
	@Override
	public boolean setValue(Object valueObject, Object value) {
		ResultsGroup results = (ResultsGroup) valueObject;
		results.setNote(value.toString());
		results.trySaveToFile();
