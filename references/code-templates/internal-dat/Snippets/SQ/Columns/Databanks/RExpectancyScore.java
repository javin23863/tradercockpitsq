...
public class RExpectancyScore extends DatabankColumn {
	...
	public RExpectancyScore() {
		...
		setDependencies("NumberOfTrades", "RExpectancy", "AvgTradesPerMonth", "AvgLoss");
	}
	...
	public double compute(SQStats stats, StatsTypeCombination combination, OrdersList ordersList, SettingsMap settings, SQStats statsLong, SQStats statsShort) throws Exception {
		...
		double rExpectancy  = stats.getDouble("RExpectancy");
		...
		double rexpectancyscore = 0;
		
	    if(numberOfTrades != 0 && Math.abs(avgLoss) != 0) {
        
        	// R-expectancy times number of trades per year
        	rexpectancyscore = rExpectancy * avgTradesPerMonth * 12f;
        }
	    
		return round2(rexpectancyscore);
	}		
