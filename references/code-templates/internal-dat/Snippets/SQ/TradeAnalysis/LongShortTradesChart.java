...
public class LongShortTradesChart extends TradeAnalysisChart {
	...
	public AbstractChart draw(OrdersList orders, byte plType, byte tradePeriod) {
		...
		chart.addValue(L.tsq("Long trades"), trades[0], ChartsConst.COLOR_BLUE);
		chart.addValue(L.tsq("Short trades"), trades[1], ChartsConst.COLOR_RED);
		
		return chart;
	}

	private int[] computeData(OrdersList orders, byte plType) {
		int[] trades = new int[2]; //long, short
