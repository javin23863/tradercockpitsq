...
public class PLbyWeekdayChart extends TradeAnalysisChart {

	public PLbyWeekdayChart() {
		this.name = L.tsq("P/L by weekday");
	}
	...
	public AbstractChart draw(OrdersList orders, byte plType, byte tradePeriod) {
		...
		for(int day=0; day<days.length; day++) {
			chart.addValue(L.tsq("P/L"), dayToString(day), days[day]);
		}
	...
	private double[] computeData(OrdersList orders, byte plType, byte tradePeriod) {
		...
		for(int i = 0; i < orders.size(); i++) {
			...
			days[day]+= order.getPLByType(plType);
		}
