...
public class WinLossPLByWeekdayChart extends TradeAnalysisChart {
	...
	public AbstractChart draw(OrdersList orders, byte plType, byte tradePeriod) {
		...
		for(int day=0; day<days.length; day++) {
			chart.addValue("winners", dayToString(day), days[day][0]);
			chart.addValue("losers", dayToString(day), days[day][1]);
		}
	...
	private double[][] computeData(OrdersList orders, byte plType, byte tradePeriod) {
		...
		for(int i = 0; i < orders.size(); i++) {
			...
			day = SQTime.getDayOfWeekOriginal(order.getTimeByPeriodType(tradePeriod)); //0-6
