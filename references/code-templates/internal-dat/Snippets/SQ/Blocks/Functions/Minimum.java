...
@BuildingBlock(name="(MIN) Minimum", display="Min(#Value1#, #Value2#)", returnType = ReturnTypes.PriceOrNumber)
...
public class Minimum extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		return Math.min(Value1.evaluateBlock(relativeShift), Value2.evaluateBlock(relativeShift));
	}
