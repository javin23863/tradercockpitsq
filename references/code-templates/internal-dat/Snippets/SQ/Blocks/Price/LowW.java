...
@BuildingBlock(name="(WL) Weekly Low", returnType=ReturnTypes.Price, display="LowW[@Chart@#Shift#]")
@SortOrder(700)
@OppositeBlock("HighW")
public class LowW extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {			
			...
			return Strategy.Stockpicker.data.LowW(chartIndex, relativeShift + Shift);
		}
		...
		if(Engines.isTradestationEngine(Strategy.getEngine())) {
			...
			if (Shift == 0 && weeksOffset == 0 && relativeShift != 0) {
				...
				if (barsShiftedSeries.get(relativeShift) == 0) {
					...
					double lowW = calculateLowDUpToBar(barIndex, maxIndex);
					barsShiftedSeries.set(relativeShift, lowW);
		            return lowW;
				}
				return barsShiftedSeries.get(relativeShift);
			}else {
				return Chart.LowW(Shift + weeksOffset);
			}
		}
		else {
			return Chart.LowW(Shift + relativeShift);
		}
	}
