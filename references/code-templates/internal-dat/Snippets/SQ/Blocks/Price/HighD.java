...
@BuildingBlock(name="(DH) Daily High", returnType=ReturnTypes.Price, display="HighD[@Chart@#Shift#]")
@SortOrder(600)
@OppositeBlock("LowD")
public class HighD extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {			
			...
			return Strategy.Stockpicker.data.HighD(chartIndex, relativeShift + Shift);
		}
		...
		if(Strategy.isTradestationEngine()) {
			...
			if (Shift == 0 && daysOffset == 0 && relativeShift != 0) {
				...
				if (barsShiftedSeries.get(relativeShift) == 0) {
					...
					double highD = calculateHighDUpToBar(barIndex, maxIndex);
					barsShiftedSeries.set(relativeShift, highD);
		            return highD;
				}
				return barsShiftedSeries.get(relativeShift);
			}else {
				return Chart.HighD(Shift + daysOffset);
			}
		}
		else {
			return Chart.HighD(Shift + relativeShift);
		}
	}
