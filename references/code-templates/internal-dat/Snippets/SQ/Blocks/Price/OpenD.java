...
@BuildingBlock(name="(DO) Daily Open", returnType=ReturnTypes.Price, display="OpenD[@Chart@#Shift#]")
@SortOrder(500)
public class OpenD extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		...
		if(Engines.isTradestationEngine(Strategy.getEngine())) {
			...
			return Chart.OpenD(Shift + daysOffset);
		}
		else {
			return Chart.OpenD(Shift + relativeShift);
		}
	}
