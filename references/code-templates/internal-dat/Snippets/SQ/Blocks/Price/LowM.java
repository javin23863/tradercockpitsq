...
@BuildingBlock(name="(ML) Monthly Low", returnType=ReturnTypes.Price, display="LowM[@Chart@#Shift#]")
@SortOrder(700)
@OppositeBlock("HighM")
public class LowM extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		if(Strategy.isStockpicker()) {			
			...
			return Strategy.Stockpicker.data.LowM(chartIndex, relativeShift + Shift);
		}
		...
		if(Engines.isTradestationEngine(Strategy.getEngine())) {
			...
			if (Shift == 0 && monthsOffset == 0 && relativeShift != 0) {
				...
				if (barsShiftedSeries.get(relativeShift) == 0) {
					...
					double lowM = calculateLowDUpToBar(barIndex, maxIndex);
					barsShiftedSeries.set(relativeShift, lowM);
		            return lowM;
				}
				return barsShiftedSeries.get(relativeShift);
			}else {
				return Chart.LowM(Shift + monthsOffset);
			}
		}
		else {
			return Chart.LowM(Shift + relativeShift);
		}
	}
