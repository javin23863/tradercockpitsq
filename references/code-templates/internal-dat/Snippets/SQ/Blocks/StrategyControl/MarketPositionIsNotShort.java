...
@BuildingBlock(display="Market Position(#Symbol#, #MagicNumber#, \"#Comment#\") is Not Short", returnType = ReturnTypes.Boolean)
@Help("Is triggered if there is no Short live order. It doesn't consider pending orders")
@SortOrder(300)
@OppositeBlock("MarketPositionIsNotLong")
@IgnoreInBuilder
public class MarketPositionIsNotShort extends ConditionBlock {
	...
	public boolean OnBlockEvaluate() throws TradingException {
		for(int i=0; i<Strategy.Trader.getOpenOrdersCount(true); i++) {
			...
			if(OrderFunctions.identify(order, Strategy, Symbol, 0, MagicNumber, Comment)) {
				if(order.getDirection() == Directions.Short) return false;
			}
		}
