...
@BuildingBlock(name="Open P/L (in pips)", display="Open P/L in pips(#Symbol#, #MagicNumber#, #Direction#, \"#Comment#\")", returnType = ReturnTypes.Number)
@Help("Returns open P/L for specified order in pips. If you'll not specify any order, it will return sum of open P/L for all active orders.")
@SortOrder(600)
@IgnoreInBuilder
@ForEngine("*,-MC,-TS,-SP,-SA")
public class OpenPLInPips extends ValueBlock {
	...
	public double OnBlockEvaluate(int relativeShift) throws TradingException {
		...
		for(int i=0; i<Strategy.Trader.getOpenOrdersCount(true); i++) {
			...
			if(OrderFunctions.identify(order, Strategy, Symbol, Direction, MagicNumber, Comment)) {
				((LiveOrderObj) order).setTickData(Strategy.Ask(), Strategy.Bid());
				plPips += order.getPLInPips();
			}
		}
