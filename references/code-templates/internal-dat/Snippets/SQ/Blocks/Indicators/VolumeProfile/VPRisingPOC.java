...
@BuildingBlock(name = "(VP21) Rising POC (Long Regime)", display = "RisingPOC(@Chart@#LookbackBars#,#MinimumShift#)", returnType = ReturnTypes.Boolean)
@Help("POC migration upward: current session POC is higher than the POC from LookbackBars ago by at least MinimumShift pips. Rising fair value across sessions suggests a bullish regime—prefer long setups on pullbacks to prior POC/value levels.")
@OppositeBlock("VPFallingPOC")
@ForEngine("MT4,MT5,TS,MC")
public class VPRisingPOC extends ConditionBlock {
    ...
    @Help("Minimum POC increase (in pips) to qualify as a rising POC")
    ...
    private boolean _evaluateSignal() throws TradingException {
        ...
        // POC has risen by at least MinimumShift
