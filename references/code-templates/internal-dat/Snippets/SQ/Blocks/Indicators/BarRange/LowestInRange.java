...
@BuildingBlock(name="Lowest in range", display="LowestInRange(@Chart@#TimeFrom#, #TimeTo#)[#Shift#]", returnType = ReturnTypes.Price)
	...
	protected void OnBarUpdate() throws TradingException {
		...
		if(currentTime >= timeTo) {
        	...
        	if(timeFrom <= currentTime) {
        		lowestValue = Input.Low(0);
        	}
		}
		else if(currentTime >= timeFrom) {
			lowestValue = Math.min(lowestValue, Input.Low(0));
		}
		else {
			lowestValue = Double.MAX_VALUE;
		}
