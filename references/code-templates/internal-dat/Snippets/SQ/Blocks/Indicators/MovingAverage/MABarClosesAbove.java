...
@BuildingBlock(name="Bar closes above Moving Average", display="Bar closes above #Type# Moving Average(@Chart@#Period#)[#Shift#]", returnType = ReturnTypes.Boolean)
@OppositeBlock("MABarClosesBelow")
...
public class MABarClosesAbove extends ConditionBlock {
	...
	public boolean OnBlockEvaluate() throws TradingException {
		...
		switch(Type){
		case 0:
			return (Strategy.Indicators.SMA(Chart.getSeries(ComputedFrom), Period).Value.getRounded(Shift) < Chart.Close(Shift));
		case 1:
			return (Strategy.Indicators.EMA(Chart.getSeries(ComputedFrom), Period).Value.getRounded(Shift) < Chart.Close(Shift));
		case 2:
			return (Strategy.Indicators.SMMA(Chart.getSeries(ComputedFrom), Period).Value.getRounded(Shift) < Chart.Close(Shift));
		case 3:
			return (Strategy.Indicators.LWMA(Chart.getSeries(ComputedFrom), Period).Value.getRounded(Shift) < Chart.Close(Shift));
		default:
			return (Strategy.Indicators.SMA(Chart.getSeries(ComputedFrom), Period).Value.getRounded(Shift) < Chart.Close(Shift));
		}
