...
@BuildingBlock(name="Bar opens above Lowest after opened below", display="Bar opens above Lowest(@Chart@#Period#)[#Shift#] after opened below", returnType = ReturnTypes.Boolean, mainIndicator="Lowest")
@OppositeBlock("BarOpensBelowHighestAfterOpenAbove")
...
public class BarOpensAboveLowestAfterOpenBelow extends ConditionBlock {
	...
	public boolean OnBlockEvaluate() throws TradingException {
		double lowest = Strategy.Indicators.Lowest(Chart.getSeries(ComputedFrom), Period).Value.getRounded(Shift+1);
		
		return (Chart.Open(Shift+1) < lowest) && (Chart.Open(Shift) > lowest);
	}
