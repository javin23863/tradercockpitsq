...
@BuildingBlock(name="Bar opens above Highest after opened below", display="Bar opens above Highest(@Chart@#Period#)[#Shift#] after opened below", returnType = ReturnTypes.Boolean, mainIndicator="Highest")
@OppositeBlock("BarOpensBelowLowestAfterOpenAbove")
...
public class BarOpensAboveHighestAfterOpenBelow extends ConditionBlock {
	...
	public boolean OnBlockEvaluate() throws TradingException {
		double highest = Strategy.Indicators.Highest(Chart.getSeries(ComputedFrom), Period).Value.getRounded(Shift+1);
		
		return (Chart.Open(Shift+1) < highest) && (Chart.Open(Shift) > highest);
	}
