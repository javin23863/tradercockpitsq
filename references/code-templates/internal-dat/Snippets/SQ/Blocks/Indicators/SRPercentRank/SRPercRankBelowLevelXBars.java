...
@BuildingBlock(name="(PRBLX) SR Percent Rank is below Level for X Bars", display=" SR Percent Rank(@Chart@#Mode#,#Length#,#ATRPeriod#)[#Shift#] is below #Level# for #Bars# Bars", returnType = ReturnTypes.Boolean)
@Help("Is triggered if SRPercentRank is below Level for X Bars")
@OppositeBlock("SRPercRankBelowLevelXBars") /// this is correct 
...
public class SRPercRankBelowLevelXBars extends ConditionBlock {
	...
	@Parameter(defaultValue="2", isPeriod=true, minValue=2, maxValue=100, step=1)
	public int Bars;

	@Parameter(defaultValue="20", isPeriod=true, minValue=0.5, maxValue=99.5, step=0.5)
	public double Level;
	...
	public boolean OnBlockEvaluate() throws TradingException {
		...
		for(int i=0; i<Bars; i++) {
			...
			if(indicator.Value.getRounded(Shift+i,5)>Level){
