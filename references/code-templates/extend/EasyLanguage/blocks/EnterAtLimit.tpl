	if(OpenOrdersAllowed) then begin        
		...
		If not LimitMaxDistanceFromMarket or Round(absvalue(IntPriceLevel<@printOrderDirection block "#Direction#" "Long" "Short" /> - <@printOrderDirection block "#Direction#" "CurrentAsk" "CurrentBid" />) / <@printOrderDirection block "#Direction#" "CurrentAsk" "CurrentBid" /> * 100, 2) <= MaxDistancePct Then begin
			...
			If UseMoneyManagement Then NumberOfShares<@printOrderDirection block "#Direction#" "Long" "Short" /> = <@printSizeMethod block "LIMIT" "#Size#" "#Direction#" "#Price#" "#StopLoss.StopLoss#" "#Symbol#" /> 
			Else NumberOfShares<@printOrderDirection block "#Direction#" "Long" "Short" /> = mmLotsIfNoMM;
			
			<#else>
			NumberOfShares<@printOrderDirection block "#Direction#" "Long" "Short" /> = <@printSizeMethod block "LIMIT" "#Size#" "#Direction#" "#Price#" "#StopLoss.StopLoss#" "#Symbol#" />;
			</#if>
			If NumberOfShares<@printOrderDirection block "#Direction#" "Long" "Short" /> > 0 Then begin
				<@printAllowDuplicateTradesCondition block "#AllowDuplicateTrades#" /> begin
					<@printOrderDirection block "#Direction#" "Buy" "SellShort" /> NumberOfShares<@printOrderDirection block "#Direction#" "Long" "Short" /> shares next bar at IntPriceLevel<@printOrderDirection block "#Direction#" "Long" "Short" /> limit;

					If UseInitialStopLoss Then begin
						<@printInitialSL block "LIMIT" />
						<@printInitialPT block "LIMIT" />
					end;
				end;
			end;
		end;
