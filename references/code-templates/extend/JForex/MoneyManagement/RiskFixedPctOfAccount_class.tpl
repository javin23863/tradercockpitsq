...
private double sqMMRiskFixedAccountPct(String symbol, OrderCommand orderType, double price, double sl, double RiskInPercent, int Decimals, int StopLossPips, double LotsIfNoMM, double MaximumLots) throws JFException {
   print("Computing Money Management for order -  Risk fixed % of account");
   ...
   if(sl == 0){
      sl = openPrice - StopLossPips * instrument.getPipValue();
   }
   
   //Maximum amount of money to risk 
   double moneyToRisk = context.getAccount().getEquity() * RiskInPercent / 100;
