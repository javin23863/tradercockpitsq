...
//+------------------------------------------------------------------+
//| Canvas-based display for Volume Profile / TPO indicators.        |
...
//--- Maximum sessions to keep for display
#define VP_MAX_HIST 50

//--- Display session history structure
...
//+------------------------------------------------------------------+
//| Initialize display system. Call from OnInit().                    |
//+------------------------------------------------------------------+
void VPDisplayInit(string prefix, int maxSessions)
{
   ...
   ObjectCreate(0, g_vpBmpName, OBJ_BITMAP_LABEL, 0, 0, 0);
   ObjectSetInteger(0, g_vpBmpName, OBJPROP_CORNER, CORNER_LEFT_UPPER);
   ObjectSetInteger(0, g_vpBmpName, OBJPROP_XDISTANCE, 0);
   ObjectSetInteger(0, g_vpBmpName, OBJPROP_YDISTANCE, 0);
   ObjectSetInteger(0, g_vpBmpName, OBJPROP_BACK, true);
...
//+------------------------------------------------------------------+
//| Cleanup all display objects. Call from OnDeinit().                |
//+------------------------------------------------------------------+
void VPDisplayDeinit()
{
   ResourceFree(g_vpResName);
   ObjectDelete(0, g_vpBmpName);
...
void VPDisplayOnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   {
      // Only re-render when the chart view actually changed (scroll/zoom/resize)
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      if(sparam == g_vpPrefix + "ToggleBtn")
      {
         ...
         ObjectSetString(0, btnName, OBJPROP_TEXT, "VP");
         ObjectSetInteger(0, btnName, OBJPROP_BGCOLOR, g_vpDisplayEnabled ? C'40,80,40' : C'80,40,40');

         if(g_vpDisplayEnabled)
         {
            ObjectSetInteger(0, g_vpBmpName, OBJPROP_TIMEFRAMES, OBJ_ALL_PERIODS);
            if(g_vpSesCount > 0) VPDisplayRender();
         }
         else
         {
            ObjectSetInteger(0, g_vpBmpName, OBJPROP_TIMEFRAMES, OBJ_NO_PERIODS);
            VPDeleteAllLabels();
         }
         ChartRedraw(0);
      }
...
//+------------------------------------------------------------------+
//| Push a VP session into display history (data only, deferred draw)|
//+------------------------------------------------------------------+
void VPDisplayPushSession(
   ...
   int numBins,
   const double &volBins[], const double &bullBins[], const double &bearBins[],
   int maxDisplay)
{
   for(int i = g_vpSesCount - 1; i >= MathMax(0, g_vpSesCount - 8); i--)
   {
      if(g_vpSessions[i].sesStart == sesStart)
      {
         FillSessionData(g_vpSessions[i], sesStart, sesEnd, sessionHigh, sessionLow,
                         ...
                         volBins, bullBins, bearBins, false);
         g_vpDirty = true;
         VPThrottledRender();
         return;
      }
   ...
   {
      for(int i = 0; i < g_vpSesCount - 1; i++)
...
//+------------------------------------------------------------------+
//| Push a TPO session into display history (data only, deferred draw)|
//+------------------------------------------------------------------+
   ...
   for(int i = g_vpSesCount - 1; i >= MathMax(0, g_vpSesCount - 8); i--)
   {
      if(g_vpSessions[i].sesStart == sesStart)
      {
         FillTPOSessionData(g_vpSessions[i], sesStart, sesEnd, sessionHigh, sessionLow,
                            poc, vah, val, ibh, ibl, numBins, tpoBins);
         g_vpDirty = true;
         VPThrottledRender();
         return;
      }
...
//+------------------------------------------------------------------+
//| Fill session data structure (unchanged from original)             |
//+------------------------------------------------------------------+
void FillSessionData(VPDisplaySession &ses,
                     ...
                     const double &hvn[], const double &lvn[],
                     int numBins,
                     const double &volBins[], const double &bullBinsA[], const double &bearBinsA[],
                     bool isTPO)
{
   ...
   for(int j = 0; j < numBins; j++)
   {
      ...
      ses.bullBins[j]   = bullBinsA[j];
      ses.bearBins[j]   = bearBinsA[j];
      ses.totalVolume     += volBins[j];
      ses.totalBullVolume += bullBinsA[j];
      ses.totalBearVolume += bearBinsA[j];
   }
}

//+------------------------------------------------------------------+
//| Copy session data (handles dynamic arrays)                        |
//+------------------------------------------------------------------+
...
void FillTPOSessionData(VPDisplaySession &ses,
                        ...
                        double poc, double vah, double val,
                        double ibh, double ibl,
                        int numBins, const int &tpoBinsA[])
{
   ses.sesStart    = sesStart;
   ses.sesEnd      = sesEnd;
   ses.sessionHigh = sessionHigh;
   ...
   for(int j = 0; j < numBins; j++)
      ses.tpoBins[j] = tpoBinsA[j];
}

//+------------------------------------------------------------------+
//| Throttled render — max once per 500ms during batch OnCalculate   |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Main canvas render — draws all sessions to a single bitmap       |
//+------------------------------------------------------------------+
void VPDisplayRender()
{
   ...
   // Pass 1: VA shading (back-most layer)
   for(int s = startSes; s < g_vpSesCount; s++)
   {
      int nBins = g_vpSessions[s].numBins;
      if(nBins <= 0) continue;
      double sH = g_vpSessions[s].sessionHigh;
      double sL = g_vpSessions[s].sessionLow;
      if(sH <= sL) continue;
...
//+------------------------------------------------------------------+
//| Update level labels (always visible, even when histogram is off)  |
//+------------------------------------------------------------------+
void VPUpdateLabels()
{
   VPDeleteAllLabels();
   ...
   for(int s = startSes; s < g_vpSesCount; s++)
   {
      ...
      if(g_vpShowStats && g_vpDisplayEnabled && !g_vpSessions[s].isTPO)
      {
         ...
         // Compute line spacing based on visible price range
         ...
         // Anchor at top of visible chart, stacking downward
         double topY = prMax - lineStep * 1.5;

         // Adaptive text color based on chart background brightness
         ...
         // Labels stacked top-to-bottom from chart top
...
//+------------------------------------------------------------------+
//| Render VP histogram bars for one session                          |
//+------------------------------------------------------------------+
void VPRenderVPBars(int idx, int cW, int cH)
{
   ...
   // Get Y mapping: two reference points
   int topY, botY;
...
//+------------------------------------------------------------------+
//| Render TPO histogram bars for one session                         |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Fill a rectangle in the pixel buffer (with clamping)              |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Draw a dotted vertical line in the pixel buffer                   |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Draw a dashed rectangle border in the pixel buffer                |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Format volume for compact display (e.g. 1.5K, 2.3M)              |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Delete all label objects (prefix + "L")                           |
//+------------------------------------------------------------------+
...
void VPCreateToggleButton()
{
   ...
   ObjectSetInteger(0, btnName, OBJPROP_CORNER, CORNER_LEFT_LOWER);
   ObjectSetInteger(0, btnName, OBJPROP_XDISTANCE, (int)(ChartGetInteger(0, CHART_WIDTH_IN_PIXELS) / 2 - 27));
   ObjectSetInteger(0, btnName, OBJPROP_YDISTANCE, 25);
   ObjectSetInteger(0, btnName, OBJPROP_XSIZE, 55);
   ...
   ObjectSetInteger(0, btnName, OBJPROP_BGCOLOR, C'80,40,40');
   ObjectSetInteger(0, btnName, OBJPROP_BORDER_COLOR, C'60,60,60');
